...
| 窗口 | 用途 | 默认尺寸 | 行为 |
| --- | --- | ---: | --- |
| `pet` | 桌面精灵本体 | `180 x 180` | 透明、无边框、置顶 |
| `bubble` | 快速对话和确认卡 | `380 x 230` | 按需显示、置顶 |
| `panel` | 历史、提醒、权限和设置 | `420 x 760` | 普通管理窗口 |
| `capture` | 截图选择蒙层 | 全屏 | 后续实现 |
...
M0 已完成。M1 已实现精灵位置恢复、拖动后边缘吸附、气泡跟随显示、精灵状态 reducer 和互动活泼度本地持久化。M2 已开始接入 OpenAI-Compatible Provider、系统安全存储、模型列表探测和 SSE 流式聊天。
