...
- OpenAI Compatible Tool Use：模型可自主调用插件；只读工具自动回传，写入工具仍需确认，不兼容模型自动降级为普通聊天

尚未完整实现的规划能力包括前台应用感知、多提供商原生工具调用、外部插件加载、自动更新、首次启动引导、国际化和数据导入导出。详见[后续功能演进路线图](docs/roadmap.md)。
